...
class Sikkord:
        ...
        while self.reconnect:
            ...
            try:
                w=await websockets.connect(
                    SERVER_URL,max_size=12_000_000,ping_interval=12,ping_timeout=12,
                    compression=None,open_timeout=12,close_timeout=2
                )
                ...
                if isinstance(auth,bytes) or not json.loads(auth).get("ok"):
                    await w.close(); await asyncio.sleep(2); continue
    ...
    def _start_voice_worker(self,mode="server",peer=None):
        if not self.voice_ready.wait(timeout=2):
            if self.loop and self.token:
                asyncio.run_coroutine_threadsafe(self.connect_media("voice"),self.loop)
            if not self.voice_ready.wait(timeout=8):
                self.ui(lambda:messagebox.showerror("Ses","Ses bağlantısı hazırlanamadı."));return
        try:
            ...
            if indev is None or indev<0:raise RuntimeError("Windows varsayılan mikrofonu bulunamadı.")
            if outdev is None or outdev<0:raise RuntimeError("Windows varsayılan hoparlörü bulunamadı.")
            rate=16000
            ...
            block=max(80,int(rate*.02));self.audio=queue.Queue(maxsize=4)
            def inp(data,frames,t,status):
                if not self.voice or self.muted:return
                try:self.raw_media("voice",b"V"+(np.clip(data[:,0],-1,1)*32767).astype(np.int16).tobytes())
                except:pass
            ...
            if mode=="dm":
                ...
                label=f"SES KANALI: {self.sn or self.sc}";self.send({"action":"voice","on":True})
            self.ui(lambda:self._voice_ui_on(label))
    ...
    # ---------- settings/profile ----------
    def settings(self):
        w=tk.Toplevel(self.r);w.title("SIKKORD • Ayarlar");w.geometry("800x720");w.configure(bg=PANEL);w.grab_set()
        ...
        tk.Label(w,text="Oturum 90 gün saklanır. X ile kapatınca SIKKORD sistem tepsisinde açık kalır.",bg=PANEL,fg=MUTED,font=("Segoe UI",9),wraplength=700,justify="left").pack(anchor="w",padx=30,pady=8)
