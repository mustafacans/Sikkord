...
def canonical_dm_room(a,b):
    x,y=sorted((str(a).lower(),str(b).lower()))
    return f"dm:{x}:{y}"
...
class Sikkord:
    def __init__(self, root):
        self.r=root
        self.r.title("SIKKORD • Premium Voice & Chat")
        ...
        self.voice_target_room=None; self.screen_target_room=None
        self.voice_room_ready=threading.Event(); self.screen_room_ready=threading.Event()
        self.voice_ctrl=queue.Queue(maxsize=5); self.screen_ctrl=queue.Queue(maxsize=5)
        self.voice_tx_packets=0; self.voice_rx_packets=0; self.last_voice_rx=0.0; self.last_voice_tx=0.0
        self.last_voice_reconnect=0.0; self.voice_watchdog_started=False
        self.call_state="idle"; self.call_peer_label=""; self.call_connected_at=0.0; self.call_status_job=None
        self.typing_last_sent=0.0; self.typing_hide_job=None
        self.brand_logo_cache={}
        self.toast_stack=[]
        ...
        self.style.configure("Ghost.TButton",background="#0d1118",foreground="#cfd6e2",borderwidth=0,padding=(12,9),font=("Segoe UI Semibold",9))
        self.style.map("Ghost.TButton",background=[("active","#171e29")],foreground=[("active","#ffffff")])
        self.style.configure("Danger.TButton",background="#8d1724",foreground="white",borderwidth=0,padding=(12,9),font=("Segoe UI Semibold",9))
        self.style.map("Danger.TButton",background=[("active",RED2)])
    ...
    async def _media_sender(self,w,kind):
        ...
        cq=self.voice_ctrl if kind=="voice" else self.screen_ctrl
        while self.reconnect:
            try:
                ctrl=cq.get_nowait()
                await w.send(json.dumps(ctrl,ensure_ascii=False,separators=(",",":")))
                continue
            except queue.Empty:
                pass
            except Exception:
            ...
            try:data=q.get_nowait()
                ...
                await asyncio.sleep(.002 if kind=="voice" else .010);continue
            try:
                await w.send(data)
                if kind=="voice":
                    self.voice_tx_packets+=1; self.last_voice_tx=time.monotonic()
            except Exception:return
    ...
    async def connect_media(self,kind):
        ...
        while self.reconnect:
            ...
            try:
                ...
                ready.set()
                target=self.voice_target_room if kind=="voice" else self.screen_target_room
                room_event=self.voice_room_ready if kind=="voice" else self.screen_room_ready
                room_event.clear()
                if target:
                    await w.send(json.dumps({"action":"media_join","room":target},separators=(",",":")))
                sender_task=asyncio.create_task(self._media_sender(w,kind))
                async for raw in w:
                    if isinstance(raw,str):
                        try:ctrl=json.loads(raw)
                        except Exception:continue
                        if ctrl.get("type")=="media_joined" and ctrl.get("ok"):
                            actual=ctrl.get("room")
                            expected=self.voice_target_room if kind=="voice" else self.screen_target_room
                            if actual==expected:room_event.set()
                        continue
                ...
                if kind=="voice":
                    self.voice_ws=None; self.voice_room_ready.clear()
                else:
                    self.screen_ws=None; self.screen_room_ready.clear()
    ...
    def set_media_room(self,kind,room):
        if kind=="voice":
            self.voice_target_room=room;self.voice_room_ready.clear();q=self.voice_ctrl
        else:
            self.screen_target_room=room;self.screen_room_ready.clear();q=self.screen_ctrl
        self._queue_latest(q,{"action":"media_join","room":room})

    def force_media_reconnect(self,kind,reason=""):
        nowm=time.monotonic()
        if kind=="voice" and nowm-self.last_voice_reconnect<7.0:return
        if kind=="voice":self.last_voice_reconnect=nowm
        w=self.voice_ws if kind=="voice" else self.screen_ws
        if self.loop and w:
            try:asyncio.run_coroutine_threadsafe(w.close(),self.loop)
            except Exception:pass
        if kind=="voice" and reason:
            self.ui(lambda:self.show_toast("Ses bağlantısı yenileniyor",reason,"warning"))

    def voice_watchdog(self):
        if not self.reconnect:return
        try:
            if self.voice:
                remote_expected=bool(self.active_dm_call) or any(m.get("voice") and not m.get("muted") and m.get("username")!=self.user for m in self.members)
                if remote_expected and self.call_connected_at and time.monotonic()-self.call_connected_at>4:
                    if self.last_voice_rx and time.monotonic()-self.last_voice_rx>4.5:
                        self.force_media_reconnect("voice","Karşı taraftan ses paketi gelmedi; otomatik yeniden bağlanılıyor.")
        except Exception:pass
        try:self.r.after(1500,self.voice_watchdog)
        except Exception:pass
    ...
    def put_audio(self,speaker,data):
        self.voice_rx_packets+=1;self.last_voice_rx=time.monotonic()
        try:
    ...
    # ---------- premium UI helpers ----------
    def brand_logo(self,size=46):
        if size in self.brand_logo_cache:return self.brand_logo_cache[size]
        try:
            im=Image.open(resource_path("assets/sikkord_logo.png")).convert("RGBA")
            im=ImageOps.contain(im,(size,size),Image.Resampling.LANCZOS)
            photo=ImageTk.PhotoImage(im);self.brand_logo_cache[size]=photo;return photo
        except Exception:return None

    def show_toast(self,title,message="",kind="info",duration=3200):
        colors={"info":"#243047","success":"#123526","warning":"#4a3514","danger":"#4a1720"}
        accent={"info":"#6ea8ff","success":"#35d07f","warning":"#ffbf4b","danger":"#ff5264"}
        try:
            win=tk.Toplevel(self.r);win.overrideredirect(True);win.attributes("-topmost",True);win.configure(bg=colors.get(kind,colors["info"]))
            sw=win.winfo_screenwidth();sh=win.winfo_screenheight();win.geometry(f"350x86+{sw-375}+{sh-145-len(self.toast_stack)*92}")
            box=tk.Frame(win,bg=colors.get(kind,colors["info"]),highlightbackground=accent.get(kind,"#6ea8ff"),highlightthickness=1);box.pack(fill="both",expand=True)
            tk.Label(box,text=title,bg=box.cget("bg"),fg="#ffffff",font=("Segoe UI Semibold",10),anchor="w").pack(fill="x",padx=14,pady=(12,2))
            tk.Label(box,text=message,bg=box.cget("bg"),fg="#c6ced9",font=("Segoe UI",8),anchor="w",wraplength=320,justify="left").pack(fill="x",padx=14)
            self.toast_stack.append(win)
            win.attributes("-alpha",0.0)
            def fade(i=0):
                if not win.winfo_exists():return
                win.attributes("-alpha",min(.96,i/8*.96))
                if i<8:self.r.after(18,lambda:fade(i+1))
            fade()
            def close():
                try:win.destroy()
                except:pass
                try:self.toast_stack.remove(win)
                except:pass
            self.r.after(duration,close)
        except Exception:pass

    def composer_focus(self,on):
        try:self.composer_border.config(bg=RED2 if on else "#5d2630")
        except:pass

    def set_call_state(self,state,peer=""):
        self.call_state=state
        if peer:self.call_peer_label=peer
        if state=="connected" and not self.call_connected_at:self.call_connected_at=time.monotonic()
        if state in ("idle","ended","rejected"):
            self.call_connected_at=0.0
            if state=="idle":self.call_peer_label=""
        self.update_call_status(schedule=True)

    def update_call_status(self,schedule=False):
        if hasattr(self,"voice_strip_label"):
            if self.call_state=="calling":
                self.voice_strip_label.config(text=f"📞 ARANIYOR • {self.call_peer_label or self.sn} ...",fg="#ffbf4b")
            elif self.call_state=="ringing":
                self.voice_strip_label.config(text=f"🔔 GELEN ARAMA • {self.call_peer_label}",fg="#ffbf4b")
            elif self.call_state=="connected":
                dur=int(max(0,time.monotonic()-self.call_connected_at)) if self.call_connected_at else 0
                self.voice_strip_label.config(text=f"🟢 ARAMA BAĞLI • {self.call_peer_label or self.sn} • {dur//60:02d}:{dur%60:02d}",fg=GREEN)
        if schedule and self.call_status_job is None:
            try:self.call_status_job=self.r.after(1000,self._call_status_tick)
            except Exception:pass

    def _call_status_tick(self):
        self.call_status_job=None
        self.update_call_status(schedule=False)
        if self.call_state in ("calling","ringing","connected"):
            try:self.call_status_job=self.r.after(1000,self._call_status_tick)
            except Exception:pass

    def typing_event(self,event=None):
        if not self.user:return
        nowm=time.monotonic()
        if nowm-self.typing_last_sent<1.1:return
        self.typing_last_sent=nowm
        if self.current_mode=="server" and self.sc:self.send({"action":"typing","scope":"server"})
        elif self.current_mode=="dm" and self.current_dm:self.send({"action":"typing","scope":"dm","peer":self.current_dm})

    def show_typing(self,username):
        if username==self.user or not hasattr(self,"typing_label"):return
        self.typing_label.config(text=f"✍ {username} yazıyor...",fg="#ff7b88")
        if self.typing_hide_job:
            try:self.r.after_cancel(self.typing_hide_job)
            except:pass
        self.typing_hide_job=self.r.after(2200,lambda:self.typing_label.config(text=""))
    ...
    def login_ui(self):
        self.clear()
        o=tk.Frame(self.r,bg="#050609");o.pack(fill="both",expand=True)
        # layered red-black premium background
        bgc=tk.Canvas(o,bg="#050609",highlightthickness=0);bgc.place(relx=0,rely=0,relwidth=1,relheight=1)
        for i in range(9):
            x=120+i*180;bgc.create_oval(x,-240,x+420,180,outline=("#2d0a10" if i%2 else "#19080c"),width=2)
        card=tk.Frame(o,bg="#0c0f14",highlightbackground="#2a303b",highlightthickness=1,padx=48,pady=38);card.place(relx=.5,rely=.49,anchor="center",width=510)
        logo=self.brand_logo(112)
        if logo:
            lab=tk.Label(card,image=logo,bg="#0c0f14");lab.image=logo;lab.pack(pady=(0,2))
        tk.Label(card,text="SIKKORD",bg="#0c0f14",fg="#ff4659",font=("Segoe UI Semibold",34)).pack()
        tk.Label(card,text="PREMIUM VOICE • CHAT • SCREEN",bg="#0c0f14",fg="#6e7786",font=("Segoe UI Semibold",8)).pack(pady=(2,18))
        tk.Label(card,text="Arkadaşlarınla düşük gecikmeli konuş, yazış ve ekranını paylaş.",bg="#0c0f14",fg="#b8c0cb",font=("Segoe UI",9),wraplength=390).pack(pady=(0,18))
        self.eu=ttk.Entry(card,width=40);self.eu.pack(fill="x",pady=6,ipady=3);self.eu.insert(0,"Kullanıcı adı")
        self.ep=ttk.Entry(card,width=40,show="*");self.ep.pack(fill="x",pady=6,ipady=3)
        ttk.Button(card,text="🚀  GİRİŞ YAP",style="Red.TButton",command=self.login).pack(fill="x",pady=(14,6))
        ttk.Button(card,text="✨  YENİ HESAP OLUŞTUR",style="Ghost.TButton",command=self.register).pack(fill="x",pady=4)
        self.status_label=tk.Label(card,text="● SUNUCUYA BAĞLANIYOR",bg="#0c0f14",fg="#ff5264",font=("Segoe UI Semibold",8));self.status_label.pack(pady=(16,0))
        tk.Label(o,text="SIKKORD 11 • En iyi deneyim için kulaklık önerilir",bg="#050609",fg="#3f4754",font=("Segoe UI",8)).pack(side="bottom",pady=16)
    ...
    def main_ui(self):
        ...
        brand=tk.Frame(top,bg="#0d0f12");brand.pack(side="left",padx=(12,20),fill="y")
        logo=self.brand_logo(46)
        if logo:
            bl=tk.Label(brand,image=logo,bg="#0d0f12");bl.image=logo;bl.pack(side="left",padx=(0,8),pady=7)
        bt=tk.Frame(brand,bg="#0d0f12");bt.pack(side="left",fill="y")
        tk.Label(bt,text="SIKKORD",bg="#0d0f12",fg=RED2,font=("Segoe UI Semibold",18)).pack(anchor="w",pady=(8,0))
        tk.Label(bt,text="PREMIUM • VOICE • CHAT",bg="#0d0f12",fg="#596273",font=("Segoe UI Semibold",7)).pack(anchor="w")
        self.status_label=tk.Label(top,text="● BAĞLI",bg="#0d0f12",fg=GREEN,font=("Segoe UI Semibold",9));self.status_label.pack(side="left")
        ttk.Button(top,text="⚙  AYARLAR",style="Ghost.TButton",command=self.settings).pack(side="right",padx=8,pady=12)
        ttk.Button(top,text="＋  SUNUCU",style="Ghost.TButton",command=self.create_server).pack(side="right",padx=2)
        ttk.Button(top,text="🔗  KATIL",style="Ghost.TButton",command=self.join_server).pack(side="right",padx=2)
        ...
        ttk.Button(nav,text="💬  SUNUCULAR",style="Ghost.TButton",command=self.open_servers_home).pack(fill="x",padx=8,pady=4)
        ttk.Button(nav,text="👥  ARKADAŞLAR",style="Ghost.TButton",command=self.show_friends).pack(fill="x",padx=8,pady=4)
        ...
        self.server_menu_btn=ttk.Button(head,text="⋯  SUNUCU",style="Ghost.TButton",command=self.server_menu);self.server_menu_btn.pack(side="right",padx=4,pady=10)
        ...
        self.show_call_btn=ttk.Button(self.voice_strip,text="👁  GÖSTER",style="Ghost.TButton",command=self.show_call_screen,state="disabled")
        self.show_call_btn.pack(side="right",padx=(4,6),pady=5)
        ttk.Button(self.voice_strip,text="📵  KAPAT",style="Danger.TButton",command=self.end_active_call).pack(side="right",padx=(6,2),pady=5)
        ...
        meta_row=tk.Frame(controls,bg=PANEL);meta_row.pack(fill="x",padx=14)
        self.reply_label=tk.Label(meta_row,text="",bg=PANEL,fg="#ff9aa2",font=("Segoe UI",8));self.reply_label.pack(side="left")
        self.typing_label=tk.Label(meta_row,text="",bg=PANEL,fg="#ff7b88",font=("Segoe UI",8));self.typing_label.pack(side="right")
        self.composer_border=tk.Frame(controls,bg="#5d2630",padx=2,pady=2);self.composer_border.pack(side="left",fill="x",expand=True,padx=(12,6),pady=12)
        self.msg=tk.Entry(self.composer_border,bg="#0d1219",fg=TEXT,insertbackground="#ff5264",relief="flat",font=("Segoe UI",11))
        self.msg.pack(fill="x",expand=True,ipady=10,padx=1,pady=1);self.msg.insert(0,"Mesaj yaz...")
        self.msg.bind("<FocusIn>",lambda e:[self.msg.delete(0,"end") if self.msg.get()=="Mesaj yaz..." else None,self.composer_focus(True)])
        self.msg.bind("<FocusOut>",lambda e:self.composer_focus(False))
        self.msg.bind("<KeyRelease>",self.typing_event)
        self.msg.bind("<Return>",lambda e:(self.send_message(),"break")[1]);self.msg.bind("<Control-v>",self.ctrl_v_image)
        ttk.Button(controls,text="📎",style="Ghost.TButton",command=self.attach_file).pack(side="left",padx=4)
        ttk.Button(controls,text="➤  GÖNDER",style="Red.TButton",command=self.send_message).pack(side="left",padx=4)
        self.vb=ttk.Button(controls,text="🎧  KATIL",style="Ghost.TButton",command=self.toggle_voice);self.vb.pack(side="left",padx=4)
        self.mb=ttk.Button(controls,text="🎙  MİK",style="Ghost.TButton",command=self.toggle_mute);self.mb.pack(side="left",padx=4)
        self.sb=ttk.Button(controls,text="🖥  EKRAN",style="Ghost.TButton",command=self.toggle_screen);self.sb.pack(side="left",padx=(4,10))
        ...
        if not self.voice_watchdog_started:
            self.voice_watchdog_started=True
            self.r.after(1500,self.voice_watchdog)
    ...
    def start_voice(self,mode="server",peer=None):
        ...
        room=canonical_dm_room(self.user,peer) if mode=="dm" and peer else ("srv:"+self.sc if self.sc else None)
        self.set_media_room("voice",room)
    ...
    def _start_voice_worker(self,mode="server",peer=None):
        ...
        if self.voice_target_room and not self.voice_room_ready.wait(timeout=5):
            self.force_media_reconnect("voice","Ses odasına bağlanma onayı alınamadı.")
            if not self.voice_room_ready.wait(timeout=5):
                with self.voice_start_lock:self.voice_starting=False
                self.ui(lambda:messagebox.showerror("Ses","Ses odasına girilemedi. Bağlantı otomatik yenilendi; tekrar deneyin."));return
        try:
            ...
            self.last_voice_rx=time.monotonic()
            if mode!="dm":
                self.call_connected_at=time.monotonic()
                self.ui(lambda:self.set_call_state("connected",self.sn))
    ...
    def stop_voice(self,notify=True):
        ...
        self.set_media_room("voice",None)
    ...
    def call(self):
        if self.current_mode=="dm" and self.current_dm:
            peer=self.current_dm
            self.set_call_state("calling",peer)
            self.set_media_room("voice",canonical_dm_room(self.user,peer))
            self.send({"action":"dm_call_start","peer":peer});self.start_voice("dm",peer)
            return
        if self.current_mode=="server" and self.sc:
            self.open_call_stage();self.set_call_state("calling",self.sn)
            self.set_media_room("voice","srv:"+self.sc)
    ...
    def incoming(self,username):
        self.set_call_state("ringing",username);self.start_ringtone()
        w=tk.Toplevel(self.r);w.title("SIKKORD • Gelen Grup Araması");w.geometry("470x310");w.configure(bg=PANEL);w.grab_set()
        ...
        def accept():
            self.stop_ringtone();self.set_media_room("voice","srv:"+self.sc)
            self.open_call_stage();self.send({"action":"call_answer"});self.start_voice("server")
            w.destroy()
    ...
    def incoming_dm(self,username):
        ...
        def accept():
            self.stop_ringtone();self.set_media_room("voice",canonical_dm_room(self.user,username))
            self.set_call_state("connected",username)
            self.send({"action":"dm_call_answer","peer":username});self.start_voice("dm",username);w.destroy()
        def reject():
            self.stop_ringtone();self.set_call_state("rejected",username);self.send({"action":"dm_call_reject","peer":username});w.destroy()
        ttk.Button(f,text="KABUL ET",style="Red.TButton",command=accept).pack(side="left",padx=8)
        ttk.Button(f,text="REDDET",command=reject).pack(side="left",padx=8);w.protocol("WM_DELETE_WINDOW",reject)
    ...
    def start_screen(self):
        ...
        self.set_media_room("screen","srv:"+self.sc)
        self.ensure_media("screen")
    ...
    def _start_screen_worker(self):
        if not self.screen_ready.wait(timeout=8):
            ...
            return
        if self.screen_target_room and not self.screen_room_ready.wait(timeout=5):
            self.force_media_reconnect("screen")
            if not self.screen_room_ready.wait(timeout=5):
                self.screen_starting=False
                self.ui(lambda:messagebox.showerror("Ekran paylaşımı","Ekran odasına bağlanılamadı."));return
        self.screen=True
        self.screen_starting=False
    ...
    def stop_screen(self):
        ...
        if was_screen:
            self.send({"action":"screen","on":False})
        self.set_media_room("screen",None)
        self.hide_share_banner()
    ...
    def settings(self):
        w=tk.Toplevel(self.r);w.title("SIKKORD 11 • Premium Ayarlar");w.geometry("980x760");w.minsize(900,700);w.configure(bg=PANEL);w.grab_set()
        ...
        diag=tk.Frame(audio,bg=PANEL2);diag.pack(fill="x",pady=(12,2))
        ttk.Button(diag,text="🔊 HOPARLÖRÜ TEST ET",style="Ghost.TButton",command=self.test_output_device).pack(side="left",padx=(0,6))
        ttk.Button(diag,text="🎙 MİKROFONU TEST ET",style="Ghost.TButton",command=self.test_microphone).pack(side="left")
        self.audio_diag_label=tk.Label(audio,text="Ses tanılama hazır.",bg=PANEL2,fg="#718096",font=("Segoe UI",8));self.audio_diag_label.pack(anchor="w",pady=(4,8))
    ...
    def test_output_device(self):
        def worker():
            try:
                ...
                rate=48000;t=np.linspace(0,.22,int(rate*.22),endpoint=False)
                tone=(np.sin(2*np.pi*660*t)*0.12).astype(np.float32)
                sd.play(tone,rate,device=dev,blocking=True)
                self.ui(lambda:self.audio_diag_label.config(text="✓ Hoparlör testi başarılı",fg=GREEN) if hasattr(self,"audio_diag_label") else None)
            except Exception as e:self.ui(lambda:self.audio_diag_label.config(text=f"Hoparlör hatası: {e}",fg=RED2) if hasattr(self,"audio_diag_label") else None)
        threading.Thread(target=worker,daemon=True).start()

    def test_microphone(self):
        def worker():
            try:
                dev=self.input_device if self.input_device is not None else sd.default.device[0]
                rate=int(float(sd.query_devices(int(dev),"input").get("default_samplerate",48000)))
                data=sd.rec(int(rate*1.0),samplerate=rate,channels=1,dtype="float32",device=dev,blocking=True)
                rms=float(np.sqrt(np.mean(data*data)+1e-9));pct=min(100,int(rms*500))
                self.ui(lambda:self.audio_diag_label.config(text=f"✓ Mikrofon alınıyor • seviye %{pct}",fg=GREEN) if hasattr(self,"audio_diag_label") else None)
            except Exception as e:self.ui(lambda:self.audio_diag_label.config(text=f"Mikrofon hatası: {e}",fg=RED2) if hasattr(self,"audio_diag_label") else None)
        threading.Thread(target=worker,daemon=True).start()
    ...
    def handle(self,m):
        ...
        if t=="login":
            ...
            scope=m.get("scope")
            if (scope=="server" and self.current_mode=="server") or (scope=="dm" and self.current_mode=="dm"):
                self.ui(lambda:self.show_typing(m.get("username","Birisi")))
        elif t=="attachment_data":
            self.ui(lambda:self.show_attachment(m))
        elif t=="group_call_state":
            if m.get("state")=="started":
                self.ui(lambda:[self.open_call_stage(),self.set_call_state("calling",self.sn),self.update_voice_strip()])
            elif m.get("state")=="accepted":
                self.ui(lambda:[self.set_call_state("connected",self.sn),self.update_voice_strip()])
            ...
            if state=="ringing":
                self.ui(lambda:self.set_call_state("calling",peer))
            elif state=="accepted":
                self.stop_ringtone();self.active_dm_call=peer;self.set_media_room("voice",canonical_dm_room(self.user,peer))
                self.ui(lambda:[self.set_call_state("connected",peer),self._voice_ui_on(f"ÖZEL ARAMA: {peer}"),self.show_toast("Arama bağlandı",f"{peer} ile ses bağlantısı aktif.","success")])
            elif state in ("rejected","ended"):
                self.stop_ringtone();self.active_dm_call=None
                self.ui(lambda:[self.set_call_state(state,peer),self.stop_voice(notify=False),self.show_toast("Arama", "Arama reddedildi." if state=="rejected" else "Arama sona erdi.", "warning")])
