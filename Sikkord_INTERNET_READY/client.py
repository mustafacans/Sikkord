...
class Sikkord:
    ...
    def put_video(self,speaker,data):
        self.last_video_sender=speaker;self.last_video_frame=data;self.remote_screen_until=time.monotonic()+2.0
        try:
            if self.video.full(): self.video.get_nowait()
            self.video.put_nowait((speaker,data))
        except:pass
        self.ui(self.render_shared_screen)
    ...
    def stop_screen(self):
        self.screen=False
        if hasattr(self,"sb"):self.sb.config(text="EKRAN")
        self.send({"action":"screen","on":False});self.hide_share_banner();self.close_video()

    def open_video(self):
        if self.vwin and self.vwin.winfo_exists():
            self.vwin.deiconify();return
        self.ensure_media("screen")
        self.vwin=tk.Toplevel(self.r);self.vwin.title("SIKKORD • Ekran Paylaşımı")
        self.vwin.geometry("1120x720");self.vwin.minsize(900,580);self.vwin.configure(bg="#06080b")
        top=tk.Frame(self.vwin,bg="#0e131a",height=44);top.pack(fill="x");top.pack_propagate(False)
        self.vstatus=tk.Label(top,text="Ekran paylaşımı bekleniyor",bg="#0e131a",fg=MUTED,font=("Segoe UI Semibold",9));self.vstatus.pack(side="left",padx=14)
        if self.screen:ttk.Button(top,text="PAYLAŞIMI DURDUR",style="Red.TButton",command=self.stop_screen).pack(side="right",padx=10,pady=5)
        self.vcanvas=tk.Canvas(self.vwin,bg="#05070a",highlightthickness=0);self.vcanvas.pack(fill="both",expand=True)
        self.vwin.protocol("WM_DELETE_WINDOW",self.vwin.withdraw)
        self.update_video()

    def render_shared_screen(self):
        if not self.last_video_frame:return
        try:
            im=Image.open(io.BytesIO(self.last_video_frame)).convert("RGB")
        except:return
        # Dedicated viewer: fit inside canvas without changing window requested size.
        if self.vcanvas and self.vcanvas.winfo_exists():
            cw=max(640,self.vcanvas.winfo_width());ch=max(360,self.vcanvas.winfo_height())
            fit=ImageOps.contain(im,(cw,ch),Image.Resampling.BILINEAR)
            photo=ImageTk.PhotoImage(fit);self.vphoto=photo
            self.vcanvas.delete("all");self.vcanvas.create_image(cw//2,ch//2,image=photo,anchor="center")
            if self.vstatus:self.vstatus.config(text=f"{self.last_video_sender or 'Bir kullanıcı'} ekranını paylaşıyor",fg=GREEN)
        # Call stage uses a stable canvas too.
        if self.call_stage_screen_canvas and self.call_stage_screen_canvas.winfo_exists():
            c=self.call_stage_screen_canvas;cw=max(700,c.winfo_width());ch=max(380,c.winfo_height())
            fit=ImageOps.contain(im,(cw-16,ch-16),Image.Resampling.BILINEAR)
            photo=ImageTk.PhotoImage(fit);self.call_stage_screen_photo=photo
            c.delete("empty");c.delete("screen");c.create_image(cw//2,ch//2,image=photo,anchor="center",tags="screen")
            c.delete("screen_text");c.create_text(18,18,text=f"{self.last_video_sender or 'Kullanıcı'} • ekran paylaşıyor",fill="#b9c5d6",anchor="nw",font=("Segoe UI Semibold",9),tags="screen_text")

    def clear_shared_screen(self,username=None):
        if username and self.last_video_sender and username!=self.last_video_sender:return
        self.last_video_frame=None;self.last_video_sender=None;self.remote_screen_until=0.0
        if self.call_stage_screen_canvas and self.call_stage_screen_canvas.winfo_exists():
            c=self.call_stage_screen_canvas;c.delete("screen");c.delete("screen_text");c.delete("empty")
            c.create_text(max(350,c.winfo_width()//2),max(220,c.winfo_height()//2),text="Ekran paylaşımı yok",fill="#4f5968",font=("Segoe UI Semibold",14),tags="empty")
        if self.vstatus:self.vstatus.config(text="Ekran paylaşımı bekleniyor",fg=MUTED)

    def update_video(self):
        if not self.vwin or not self.vwin.winfo_exists():return
        latest=None
        try:
            while True:latest=self.video.get_nowait()
        except queue.Empty:pass
        if latest:
            self.last_video_sender,self.last_video_frame=latest
            self.render_shared_screen()
        self.vwin.after(90,self.update_video)

    def close_video(self):
        try:
            if self.vwin:self.vwin.destroy()
        except:pass
        self.vwin=None;self.vcanvas=None;self.vphoto=None;self.vstatus=None
    ...
    def handle(self,m):
        ...
        if t=="login":
            ...
            if any(x.get("screen") and x.get("username")!=self.user for x in self.members):
                self.ensure_media("screen")
                self.ui(self.open_call_stage if self.voice else self.open_video)
            ...
            for mm in self.members:
                if mm.get("username")==m.get("username"):mm["screen"]=bool(m.get("on"))
            self._member_sig=None
            if m.get("on") and m.get("username")!=self.user:
                self.ensure_media("screen")
                if self.voice:self.ui(self.open_call_stage)
                else:self.ui(self.open_video)
            elif not m.get("on"):
                self.ui(lambda:self.clear_shared_screen(m.get("username")))
            self.ui(self.render_members)
        elif t=="profile":
            if m.get("ok"):
