...
def image_file_to_b64(path, max_side=512, quality=84):
...
class AnimatedAvatar(QWidget):
    """Circular avatar with animated green speaking glow."""
    def __init__(self, avatar=None, size=72, parent=None):
        super().__init__(parent)
        self.base_size=size
        self.avatar_pm=circular_pixmap(avatar,size)
        self._pulse=0.0
        self.speaking=False
        self.setFixedSize(size+24,size+24)
        self.anim=QPropertyAnimation(self,b"pulse",self)
    ...
    def set_speaking(self,on):
        on=bool(on)
        if on==self.speaking:return
        self.speaking=on
        if on:self.anim.start()
        else:
            self.anim.stop();self._pulse=0.0;self.update()

    def paintEvent(self,event):
        p=QPainter(self);p.setRenderHint(QPainter.Antialiasing)
        cx=self.width()/2;cy=self.height()/2
        if self.speaking:
            r=self.base_size/2+3+5*self._pulse
            p.setPen(QPen(QColor(60,240,145,int(95+90*self._pulse)),4.0))
            p.drawEllipse(QRectF(cx-r,cy-r,r*2,r*2))
            r2=r+3
            p.setPen(QPen(QColor(65,255,160,45),7.0))
            p.drawEllipse(QRectF(cx-r2,cy-r2,r2*2,r2*2))
        p.drawPixmap(int(cx-self.base_size/2),int(cy-self.base_size/2),self.avatar_pm)
        p.end()


class IdentityRow(QWidget):
    def __init__(self, username, avatar=None, subtitle="", color="#F4F5F7", online=False, parent=None):
        super().__init__(parent)
        row=QHBoxLayout(self);row.setContentsMargins(4,4,6,4);row.setSpacing(9)
        av=QLabel();av.setFixedSize(40,40);av.setPixmap(circular_pixmap(avatar,40));row.addWidget(av)
        col=QVBoxLayout();col.setSpacing(1)
        name=QLabel(username);name.setStyleSheet(f"font-weight:800;color:{color};")
        sub=QLabel(subtitle);sub.setStyleSheet("color:#788394;font-size:10px;")
        col.addWidget(name);col.addWidget(sub);row.addLayout(col,1)
        dot=QLabel("●" if online else "○");dot.setStyleSheet(f"color:{'#42E28F' if online else '#586374'};font-size:13px;");row.addWidget(dot)
...
class ServerRow(QWidget):
    def __init__(self, name, icon=None, parent=None):
        super().__init__(parent)
        row=QHBoxLayout(self);row.setContentsMargins(4,4,6,4);row.setSpacing(9)
        av=QLabel();av.setFixedSize(40,40);av.setPixmap(circular_pixmap(icon,40));row.addWidget(av)
        title=QLabel(name);title.setStyleSheet("font-weight:800;color:#E9EDF4;");row.addWidget(title,1)
        arr=QLabel("›");arr.setStyleSheet("color:#5D6878;font-size:19px;");row.addWidget(arr)
...
class MessageBubble(QFrame):
    def __init__(self, app, msg):
        ...
        outer=QHBoxLayout(self);outer.setContentsMargins(2,2,2,2);outer.setSpacing(9)

        avdata=msg.get("avatar") or app.avatar_for(who)
        av=QLabel();av.setFixedSize(40,40);av.setPixmap(circular_pixmap(avdata,40));outer.addWidget(av,0,Qt.AlignTop)

        bubble=QFrame();bubble.setObjectName("MessageBubbleSelf" if mine else "MessageBubbleOther");bubble.setMaximumWidth(940)
        lay=QVBoxLayout(bubble);lay.setContentsMargins(12,8,12,8);lay.setSpacing(4)

        top=QHBoxLayout()
        author=QLabel(who);author.setObjectName("MessageAuthor")
        author.setStyleSheet(f"font-weight:800;color:{msg.get('color') or app.color_for(who)};")
        top.addWidget(author)
        meta=QLabel(time.strftime("%H:%M",time.localtime(msg.get("created_at",time.time()))));meta.setObjectName("MessageMeta");top.addWidget(meta);top.addStretch()
        lay.addLayout(top)

        if msg.get("reply_to"):
            rr=QLabel(f"↪ #{msg['reply_to']} mesajına yanıt");rr.setObjectName("MessageMeta");lay.addWidget(rr)
        ...
        if msg.get("deleted"):
            ...
            if match:
                btn=QPushButton("🔗  SUNUCUYA KATIL");btn.setProperty("accent",True)
                btn.clicked.connect(lambda:self.app.net.send({"action":"join","code":match.group(1).upper()}));lay.addWidget(btn,0,Qt.AlignLeft)

        if msg.get("attachment_name"):
            att=QPushButton(f"📎  {msg.get('attachment_name')}  •  AÇ")
            att.clicked.connect(lambda:self.app.net.send({"action":"attachment_get","id":msg.get("id"),"scope":"dm" if app.mode=="dm" else "server"}))
        ...
        if app.mode=="dm" and msg.get("sender")==app.user:
            ...
            mr=QLabel(mark);mr.setObjectName("MessageMeta");lay.addWidget(mr,0,Qt.AlignRight)
    ...
    def menu(self,pos):
        ...
        if who==self.app.user:delete=m.addAction("🗑 Mesajı sil")
        act=m.exec(self.mapToGlobal(pos))
        if act==reply:
            self.app.reply_to=self.msg.get("id");self.app.reply_label.setText(f"↪ #{self.app.reply_to} mesajına yanıt veriyorsun")
        elif info and act==info:self.app.net.send({"action":"message_info","id":self.msg.get("id")})
        elif delete and act==delete:self.app.net.send({"action":"delete_dm" if self.app.mode=="dm" else "delete_server_message","id":self.msg.get("id")})


class ParticipantCard(QFrame):
    def __init__(self,name,avatar=None):
        super().__init__()
        self.name=name;self.setObjectName("ParticipantCard");self.setMinimumSize(185,150)
        lay=QVBoxLayout(self);lay.setAlignment(Qt.AlignCenter);lay.setSpacing(2)
        self.avatar_widget=AnimatedAvatar(avatar,68);lay.addWidget(self.avatar_widget,0,Qt.AlignCenter)
        ...
        self.state=QLabel("Aramada");self.state.setAlignment(Qt.AlignCenter);self.state.setStyleSheet("color:#42D88B;font-size:10px;");lay.addWidget(self.state)

    def set_speaking(self,on):
        self.avatar_widget.set_speaking(on)
...
class MainWindow(QMainWindow):
    ...
    def avatar_for(self, username):
        if username==self.user:return self.avatar
        for m in self.members:
            if m.get("username")==username and m.get("avatar"):return m.get("avatar")
        for f in self.friends:
            if f.get("username")==username and f.get("avatar"):return f.get("avatar")
        return None

    def color_for(self, username):
        if username==self.user:return self.user_color
        for m in self.members:
            if m.get("username")==username:return m.get("color") or "#F4F5F7"
        for f in self.friends:
            if f.get("username")==username:return f.get("color") or "#F4F5F7"
        return "#F4F5F7"

    def current_server(self):
        return next((s for s in self.servers if s.get("code")==self.server_code),None)

    def set_page_server_icon(self,data=None):
        if hasattr(self,"page_icon"):self.page_icon.setPixmap(circular_pixmap(data,38))
    ...
    def build_main(self):
        ...
        self.page_icon=QLabel();self.page_icon.setFixedSize(38,38);self.page_icon.setPixmap(circular_pixmap(None,38));hl.addWidget(self.page_icon)
    ...
    def show_servers(self):
        ...
        for s in self.servers:
            it=QListWidgetItem();it.setData(Qt.UserRole,("server",s));it.setSizeHint(QSize(225,52))
            self.side_list.addItem(it);self.side_list.setItemWidget(it,ServerRow(s.get("name","Sunucu"),s.get("icon")))
        self.page_title.setText(self.server_name or "Sunucular")
        cur=self.current_server();self.set_page_server_icon(cur.get("icon") if cur else None)
        if self.server_code:
    ...
    def show_friends(self):
        ...
        for f in self.friends:
            it=QListWidgetItem();it.setData(Qt.UserRole,("friend",f));it.setSizeHint(QSize(225,54))
            self.side_list.addItem(it)
            subtitle="Çevrimiçi" if f.get("online") else f"Son görülme: {fmt_seen(f.get('last_seen'))}"
            self.side_list.setItemWidget(it,IdentityRow(f.get("username","?"),f.get("avatar"),subtitle,f.get("color") or "#F4F5F7",bool(f.get("online"))))
        self.page_title.setText("Arkadaşlar");self.set_page_server_icon(None)
        self.clear_chat()
    ...
    def side_menu(self,pos):
        ...
        logo_action=m.addAction("🖼 Sunucu logosunu değiştir") if owner else None
        ...
        elif logo_action and act==logo_action:self.change_server_logo(data)
        elif act==danger:
    ...
    def server_context_menu_button(self):
        ...
        elif logo_action and act==logo_action:self.change_server_logo(s)
        elif act==danger:
    ...
    def create_server(self):
        d=QDialog(self);d.setWindowTitle("Yeni sunucu");d.resize(460,310)
        l=QVBoxLayout(d);l.setContentsMargins(24,22,24,22);l.setSpacing(12)
        title=QLabel("Yeni bir sunucu oluştur");title.setStyleSheet("font-size:20px;font-weight:800;");l.addWidget(title)
        e=QLineEdit();e.setPlaceholderText("Sunucu adı");l.addWidget(e)
        selected={"icon":None}
        row=QHBoxLayout();preview=QLabel();preview.setFixedSize(60,60);preview.setPixmap(circular_pixmap(None,60));row.addWidget(preview)
        choose=QPushButton("🖼 SUNUCU LOGOSU SEÇ")
        def pick():
            path,_=QFileDialog.getOpenFileName(d,"Sunucu logosu seç","","Resim (*.png *.jpg *.jpeg *.webp)")
            if not path:return
            try:
                selected["icon"]=image_file_to_b64(path)
                preview.setPixmap(circular_pixmap(selected["icon"],60))
            except Exception as ex:self.modal("Logo",str(ex),"warning")
        choose.clicked.connect(pick);row.addWidget(choose,1);l.addLayout(row)
        hint=QLabel("Logo sunucu listesinde ve başlıkta görünür.");hint.setObjectName("SubtleText");l.addWidget(hint)
        b=QPushButton("SUNUCUYU OLUŞTUR");b.setProperty("accent",True);l.addWidget(b);b.clicked.connect(d.accept)
        if d.exec() and e.text().strip():self.net.send({"action":"create","name":e.text().strip(),"icon":selected["icon"]})

    def change_server_logo(self,server_data=None):
        s=server_data or self.current_server()
        if not s or s.get("owner")!=self.user:
            self.toast("Sunucu logosu","Bunu yalnızca sunucu sahibi değiştirebilir.","warning");return
        path,_=QFileDialog.getOpenFileName(self,"Sunucu logosu seç","","Resim (*.png *.jpg *.jpeg *.webp)")
        if not path:return
        try:self.net.send({"action":"server_icon","code":s.get("code"),"icon":image_file_to_b64(path)})
        except Exception as ex:self.modal("Sunucu logosu",str(ex),"warning")
    ...
    def handle_event(self,m):
        ...
        if t=="login":
            ...
            self.server_code=m.get("code","");self.server_name=m.get("name","")
            self.servers.append({"code":self.server_code,"name":self.server_name,"owner":self.user,"icon":m.get("icon")})
            self.net.send({"action":"enter","code":self.server_code});self.net.send({"action":"servers"})
        elif t=="joined":
            if m.get("ok"):
        ...
        elif t=="server_icon_result":
            self.toast("Sunucu logosu",m.get("message",""),"info" if m.get("ok") else "warning")
            if m.get("ok"):
                for s in self.servers:
                    if s.get("code")==m.get("code"):s["icon"]=m.get("icon")
                if self.mode=="server":self.show_servers()
                if self.server_code==m.get("code"):self.set_page_server_icon(m.get("icon"))
        elif t=="server_icon_changed":
            for s in self.servers:
                if s.get("code")==m.get("code"):s["icon"]=m.get("icon")
            if self.mode=="server":self.show_servers()
            if self.server_code==m.get("code"):self.set_page_server_icon(m.get("icon"))
    ...
    def render_members(self):
        ...
        for m in self.members:
            state="Seste" if m.get("voice") else ("Çevrimiçi" if m.get("online") else f"Son görülme: {fmt_seen(m.get('last_seen'))}")
            if m.get("screen"):state+=" • Ekran paylaşıyor"
            if m.get("muted"):state+=" • Mikrofon kapalı"
            it=QListWidgetItem();it.setSizeHint(QSize(220,56));self.member_list.addItem(it)
            self.member_list.setItemWidget(it,IdentityRow(m.get("username","?"),m.get("avatar"),state,m.get("color") or "#F4F5F7",bool(m.get("online"))))
        self.refresh_call_people()
