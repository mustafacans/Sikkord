...
class NoiseSuppressor:
    """Very low-cost real-time high-pass + adaptive soft gate."""
    def __init__(self):
    ...
    def process(self, x, enabled=True, strength=70):
        ...
        if not enabled or x.size==0:
            return x
        # Vectorized high-pass. Heavy DSP never runs in the PortAudio callback.
        y=np.empty_like(x)
        y[0]=x[0]-0.97*self.prev_sample
        if x.size>1:
            y[1:]=x[1:]-0.97*x[:-1]
        self.prev_sample=float(x[-1])
        rms=float(np.sqrt(np.mean(y*y)+1e-9))
        if rms < max(self.noise_rms*2.0,0.03):
            self.noise_rms=0.985*self.noise_rms+0.015*rms
        s=max(0,min(100,int(strength)))/100.0
        threshold=max(0.0035,self.noise_rms*(1.6+1.5*s))
        ratio=rms/(threshold+1e-9)
        if ratio<0.55:
            target=0.03 + (1.0-s)*0.10
        elif ratio<1.15:
            target=0.18 + 0.82*((ratio-0.55)/0.60)
        else:
            target=1.0
        self.gain=0.88*self.gain+0.12*target
        y=y*self.gain
...
class Sikkord:
                ...
                while self.voice and not self.audio_worker_stop.is_set():
                    try:
                        ...
                        x=self.audio_fx.process(
                            ...
                            int(self.app_settings.get("noise_strength",72)),
                        )
            ...
            def playback_loop():
                while self.voice and not self.audio_worker_stop.is_set():
                    ...
                    for speaker,q in items:
                        ...
                        try:mixes.append(mulaw_decode(raw))
                        except Exception:pass
                    try:
                        if mixes:
    ...
    def screen_loop(self):
        while self.screen:
            ...
            try:
                im=ImageGrab.grab().convert("RGB");im.thumbnail((1280,720),Image.Resampling.BILINEAR)
                b=io.BytesIO();im.save(b,"JPEG",quality=68,optimize=False)
                self.raw_media("screen",b"S"+b.getvalue())
            except:break
            time.sleep(max(0,1/6-(time.perf_counter()-st)))
