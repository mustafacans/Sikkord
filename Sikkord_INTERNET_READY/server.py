            ...
            if state["kind"]=="unknown":
                ...
                if first.get("action")=="media_auth":
                    ...
                    if kind=="voice" and (int(first.get("proto",0))!=2 or first.get("codec")!="pcm16"):
                        await send(ws,{"type":"media_auth","ok":False,"message":"V8 PCM16 gerekli"}); await ws.close(); return
                    ms=clients[main]
                    ...
                    await send(ws,{"type":"media_auth","ok":True,"kind":kind,"proto":2 if kind=="voice" else 1,"codec":"pcm16" if kind=="voice" else "jpeg"})
                    continue
                state["kind"]="main"
