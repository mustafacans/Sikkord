...
class WSAdapter:
    """aiohttp WebSocketResponse'u mevcut Sikkord handler API'sine uyarlar."""
    ...
    async def __anext__(self):
        while True:
            ...
            # PING/PONG gibi kontrol mesajlarını kullanıcı handler'ına verme.
...
async def http_or_websocket(request):
    """
    Render normal HTTP GET/HEAD istekleri gönderir.
    WebSocket Upgrade varsa Sikkord bağlantısını açar;
    normal HTTP isteğinde 200 OK döndürür.
    """
...
async def on_startup(app):
    ...
    print(f"SIKKORD HTTP + WebSocket BACKEND -> {HOST}:{PORT}")


def make_app():
    app = web.Application(client_max_size=12_000_000)
    app.on_startup.append(on_startup)
    app.router.add_route("*", "/{tail:.*}", http_or_websocket)
    return app
