...
def server_history(code, limit=60):
    c = conn()
    try:
        rows = c.execute("""
            SELECT sm.id,sm.username,sm.text,sm.attachment_name,sm.attachment_mime,
                   sm.attachment_data,sm.reply_to,sm.created_at,sm.deleted,u.avatar
            FROM server_messages sm
            LEFT JOIN users u ON u.username=sm.username
            WHERE sm.server_code=%s
            ORDER BY sm.id DESC LIMIT %s
        """, (code, limit)).fetchall()
        rows.reverse()
        return [{
            "id": r[0], "username": r[1], "avatar": r[9],
            "text": "[mesaj silindi]" if r[8] else r[2],
            "message": "[mesaj silindi]" if r[8] else r[2],
...
def mark_server_read(username, code):
    ...
    try:
        rows = c.execute("""
            ...
            SELECT sm.id,%s,%s
            FROM server_messages sm
            JOIN members m ON m.server_code=sm.server_code AND m.username=%s
            WHERE sm.server_code=%s AND sm.deleted=0
            ON CONFLICT(message_id,username) DO UPDATE SET read_at=EXCLUDED.read_at
            RETURNING message_id
        """, (username, now(), username, code)).fetchall()
        c.commit()
...
def friend_list(username):
    ...
    try:
        ...
        return [{"username":u,"avatar":av,"online":u in online_users,"last_seen":seen or 0}
                for u,av,seen in rows]
    finally:
        c.close()
...
def dm_history(a, b, limit=80):
    c = conn()
    try:
        rows = c.execute("""
            SELECT dm.id,dm.sender,dm.receiver,dm.text,dm.attachment_name,dm.attachment_mime,
                   dm.attachment_data,dm.reply_to,dm.created_at,dm.delivered_at,dm.read_at,dm.deleted,u.avatar
            FROM dm_messages dm
            LEFT JOIN users u ON u.username=dm.sender
            WHERE (dm.sender=%s AND dm.receiver=%s) OR (dm.sender=%s AND dm.receiver=%s)
            ORDER BY dm.id DESC LIMIT %s
        """, (a,b,b,a,limit)).fetchall()
        rows.reverse()
        return [{
            "id":r[0],"sender":r[1],"receiver":r[2],"avatar":r[12],
            ...
            "reply_to":r[7],"created_at":r[8],"delivered_at":r[9],"read_at":r[10],"deleted":bool(r[11]),
        } for r in rows]
    finally:
        c.close()
...
async def move_user_media(token, room):
...
async def handler(ws):
    clients[ws] = {
        ...
        "media_room":None,"call_peer":None,"room":None,
    }
    try:
        async for incoming in ws:
            ...
            if state["kind"]=="unknown":
                ...
                if first.get("action")=="media_auth":
                    ...
                    ms=clients[main]
                    state.update({"kind":"media","u":ms["u"],"s":ms["s"],"token":token,"media_kind":kind,"room":ms.get("media_room")})
                    await move_media(ws,ms.get("media_room"))
            ...
            if state["kind"]=="media":
                if isinstance(incoming,bytes) and state.get("room"):
                    if state["media_kind"]=="voice" and incoming.startswith(b"V"):
                        name=state.get("u","").encode("utf-8")[:63]
                        framed=b"V"+bytes([len(name)])+name+incoming[1:]
                        await broadcast_media(state["room"],"voice",framed,ws)
                    elif state["media_kind"]=="screen" and incoming.startswith(b"S"):
                        await broadcast_media(state["room"],"screen",incoming,ws)
                continue
            ...
            if a=="servers":
                ...
                if state["voice"] and state.get("s"):
                    ...
                    await move_user_media(state["token"],state["media_room"])
                elif not state["voice"] and not state.get("call_peer"):
                    state["media_room"]=None
                    await move_user_media(state["token"],None)
                ...
                if state["screen"] and state.get("s") and not state.get("media_room"):
                    state["media_room"]="srv:"+state["s"]
                    await move_user_media(state["token"],state["media_room"])
                ...
                await move_user_media(state["token"],room)
                ...
                for ow,os_ in list(clients.items()):
                    if os_.get("kind")=="main" and os_.get("u")==peer:
                        ...
                        await move_user_media(os_.get("token"),room)
                ...
                if peer:
                    for ow,os_ in list(clients.items()):
                        if os_.get("kind")=="main" and os_.get("u")==peer and os_.get("media_room")==old_room:
                            ...
                            await move_user_media(os_.get("token"),None)
