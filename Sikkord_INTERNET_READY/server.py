    ...
    try:
        async for incoming in ws:
            ...
            if state["kind"]=="media":
                if isinstance(incoming,bytes) and state.get("room"):
                    ...
                    if incoming.startswith(prefix):
                        await broadcast_media(state["room"],state["media_kind"],incoming,ws)
                continue
            ...
            if a=="servers":
                ...
                old=state.get("s")
                state["s"]=code
                reads=mark_server_read(state["u"],code)
                ...
                mark_server_read(state["u"],state["s"])
                ...
                if state.get("s"):
                    ids=mark_server_read(state["u"],state["s"])
                    if ids:
                ...
                info=server_message_info(state.get("s"),mid) if state.get("s") else None
                await send(ws,{"type":"message_info","info":info})

            elif a=="leave_server":
                code=str(m.get("code",state.get("s") or "")).upper()
                ok,msg=leave_server(state["u"],code)
                ...
                code=str(m.get("code",state.get("s") or "")).upper()
                ok,msg=delete_server_db(state["u"],code)
                if ok and state.get("s")==code: state["s"]=None
                ...
                peer=str(m.get("peer","")).strip()
                if not is_friend(state["u"],peer): continue
                room=dm_room(state["u"],peer)
                state["call_peer"]=peer; state["media_room"]=room; state["voice"]=True
                await move_user_media(state["token"],room)
