...
def init_db():
    ...
    try:
        ...
        c.execute("ALTER TABLE servers ADD COLUMN IF NOT EXISTS icon TEXT")
...
def create_server(name, owner, icon=None):
    ...
    try:
        while c.execute("SELECT 1 FROM servers WHERE code=%s", (code,)).fetchone():
            code = new_invite_code()
        c.execute(
            "INSERT INTO servers(name,code,owner,icon,created_at) VALUES(%s,%s,%s,%s,%s)",
            (name, code, owner, icon, now()),
        )
        c.execute(
...
def update_server_icon(username, code, icon):
    c = conn()
    try:
        ...
        if row[0] != username:
            return False, "Sunucu logosunu yalnızca sunucu sahibi değiştirebilir."
        c.execute("UPDATE servers SET icon=%s WHERE code=%s", (icon or None, code))
        c.commit()
        return True, "Sunucu logosu güncellendi."
    finally:
        c.close()
...
def user_servers(username):
    ...
    try:
        rows = c.execute("""
            SELECT s.name,s.code,s.owner,s.icon FROM servers s
            JOIN members m ON m.server_code=s.code
            WHERE m.username=%s ORDER BY s.id
        """, (username,)).fetchall()
        return [{"name": n, "code": code, "owner": owner, "icon": icon} for n, code, owner, icon in rows]
    finally:
        c.close()
...
def can_join_media_room(username,room):
    room=str(room or "")
    if not room:return True
    if room.startswith("srv:"):
        code=room[4:]
        c=conn()
        try:return bool(c.execute("SELECT 1 FROM members WHERE username=%s AND server_code=%s",(username,code)).fetchone())
        finally:c.close()
    if room.startswith("dm:"):
        try:
            for item in friend_list(username):
                peer=item.get("username")
                if peer and dm_room(username,peer)==room:return True
...
async def handler(ws):
    ...
    try:
        async for incoming in ws:
            ...
            if state["kind"]=="unknown":
                ...
                if first.get("action")=="media_auth":
                    ...
                    if kind=="voice" and (int(first.get("proto",0))!=3 or first.get("codec")!="pcm16"):
                        await send(ws,{"type":"media_auth","ok":False,"message":"V11 PCM16 gerekli"}); await ws.close(); return
                    if kind=="screen" and int(first.get("proto",0))!=3:
                        await send(ws,{"type":"media_auth","ok":False,"message":"V10 ekran protokolü gerekli"}); await ws.close(); return
                    ms=clients[main]
                    ...
                    await send(ws,{"type":"media_auth","ok":True,"kind":kind,"proto":3 if kind=="voice" else 3,"codec":"pcm16" if kind=="voice" else "jpeg"})
                    continue
                state["kind"]="main"

            if state["kind"]=="media":
                if isinstance(incoming,str):
                    try:ctrl=json.loads(incoming)
                    except Exception:continue
                    if ctrl.get("action")=="media_join":
                        room=ctrl.get("room")
                        ok=await adb(can_join_media_room,state.get("u"),room)
                        if ok:
                            await move_media(ws,room);state["room"]=room
                        await send(ws,{"type":"media_joined","ok":bool(ok),"room":room if ok else None,"kind":state.get("media_kind")})
                    continue
            ...
            if a=="servers":
                ...
                if len(icon)>MAX_AVATAR:
                    await send(ws,{"type":"error","message":"Sunucu logosu çok büyük."}); continue
                code=await adb(create_server,str(m.get("name","Yeni Sunucu")),state["u"],icon or None)
                await send(ws,{"type":"created","name":str(m.get("name","Yeni Sunucu"))[:40],"code":code,"icon":icon or None})
                ...
                if scope=="dm":
                    peer=str(m.get("peer","")).strip()
                    if peer and await adb(is_friend,state["u"],peer):
                        await send_to_user(peer,{"type":"typing","scope":"dm","username":state["u"]})
                elif state.get("s"):
                    await broadcast_server(state["s"],{"type":"typing","scope":"server","username":state["u"]},ws)
                ...
                code=str(m.get("code",state.get("s") or "")).upper().strip()
                icon=str(m.get("icon","") or "")
                if len(icon)>MAX_AVATAR:
                    await send(ws,{"type":"server_icon_result","ok":False,"message":"Sunucu logosu çok büyük.","code":code}); continue
                ok,msg=await adb(update_server_icon,state["u"],code,icon or None)
                await send(ws,{"type":"server_icon_result","ok":ok,"message":msg,"code":code,"icon":icon or None})
                if ok:
                    await broadcast_server(code,{"type":"server_icon_changed","code":code,"icon":icon or None})
                ...
                await send(ws,{"type":"group_call_state","state":"started","server":state["s"]})
                await push_members(state["s"])
                ...
                if state.get("s"):
                    ...
                    state["media_room"]="srv:"+state["s"]
                    await move_user_media(state["token"],state["media_room"],"voice")
                    await broadcast_server(state["s"],{"type":"group_call_state","state":"accepted","user":state["u"]})
                    await push_members(state["s"])

            elif a=="call_reject":
                await broadcast_server(state["s"],{"type":"call_state","user":state["u"],"state":"rejected"})

            elif a=="group_call_end":
                code=state.get("s")
                if code:
                    # End the server call for all connected participants.
                    affected=[]
                    for ow,os_ in list(clients.items()):
                        if os_.get("kind")=="main" and os_.get("s")==code and os_.get("voice"):
                            os_["voice"]=False
                            os_["muted"]=False
                            os_["call_peer"]=None
                            os_["media_room"]=None
                            affected.append((ow,os_))
                    for ow,os_ in affected:
                        await move_user_media(os_.get("token"),None,"voice")
                    await broadcast_server(code,{
                ...
                state["call_peer"]=None
                state["media_room"]=None
                await move_user_media(state.get("token"),None,"voice")
        ...
        if state.get("s"):
            if state.get("screen"):
                await broadcast_server(state["s"],{"type":"screen","username":state["u"],"on":False})
            if state.get("voice"):
                await broadcast_server(state["s"],{"type":"voice","username":state["u"],"on":False})
            await push_members(state["s"])
        if state.get("u"):
...
async def on_startup(app):
    ...
    print(f"SIKKORD PREMIUM V11 -> {HOST}:{PORT}")
